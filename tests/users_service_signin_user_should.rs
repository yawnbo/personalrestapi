...
async fn return_success_when_downstream_services_succeed_and_user_exists() {
    ...
    fixture
        ...
        .with(
            eq(String::from("f3f898aa-ffa3-4b58-91b0-612a1c801a5e")),
            eq("stub email"),
        )
        .times(0)
    ...
    fixture
        .mock_sessions_services
        .expect_new_session()
        // bandaid ass fix STILL, this should actually test with something
        .times(1)
...
#[tokio::test]
async fn return_error_when_user_exists() {
    let mut fixture = UsersServiceTestFixture::default();
...
#[tokio::test]
async fn reject_password_login_for_oidc_user() {
    // arrange
    let mut fixture = UsersServiceTestFixture::default();

    // Create an OIDC user (empty password, has auth_provider)
    let oidc_user = User {
        id: String::from("oidc-user-id"),
        email: String::from("stub email"),
        name: String::from("OIDC User"),
        password: String::from(""), // Empty password indicates OIDC user
        auth_provider: Some(String::from("google")),
        ..User::default()
    };
    ...
    fixture
        ...
        .with(eq("stub email"))
        .times(1)
        .return_once(move |_| Ok(Some(oidc_user)));

    // Password verification should NOT be called for OIDC users
    fixture.mock_argon_util.expect_verify_password().times(0);

    // Session should NOT be created
    fixture.mock_sessions_services.expect_new_session().times(0);
