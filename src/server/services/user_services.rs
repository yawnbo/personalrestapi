...
pub trait UsersServiceTrait {
    ...
    async fn get_current_user(&self, user_id: String) -> AppResult<ResponseUserDto>;
...
impl UsersServiceTrait for UsersService {
    async fn signup_user(&self, request: SignUpUserDto) -> AppResult<ResponseUserDto> {
        let email = request.email.ok_or_else(|| {
            Error::InternalServerErrorWithContext("Email field missing from validated request".to_string())
        })?;
        let name = request.name.ok_or_else(|| {
            Error::InternalServerErrorWithContext("Name field missing from validated request".to_string())
        })?;
        let password = request.password.ok_or_else(|| {
            Error::InternalServerErrorWithContext("Password field missing from validated request".to_string())
        })?;

        let mut salt = [0u8; 16];
        OsRng.try_fill_bytes(&mut salt)
            .map_err(|e| Error::InternalServerErrorWithContext(
        ...
        let hashed_password = self.argon_util.hash_password(&password, &salt)?;
    ...
    async fn signin_user(
        ...
        let attempted_password = request.password.ok_or_else(|| {
            Error::InternalServerErrorWithContext("Password field missing from validated request".to_string())
        })?;
        ...
        let user = existing_user.ok_or_else(|| {
            Error::NotFound(String::from("user email does not exist"))
        })?;

        // Block password login for OIDC users
        if user.is_oidc_user() {
            error!("OIDC user attempted password login: {:?}", email);
            return Err(Error::BadRequest(
                "This account uses social login. Please sign in with your connected provider.".to_string()
            ));
        }
    ...
    async fn get_current_user(&self, user_id: String) -> AppResult<ResponseUserDto> {
        ...
        let token = self
            .jwt_util
            .new_access_token(user.id.clone(), user.email.as_str())?;
    ...
    async fn updated_user(
        ...
        // alloc space for the salt if the user wants password changed
        let mut salt = [0u8; 16];
        ...
        if let Some(ref password) = request.password {
            if !password.is_empty() {
                ...
                updated_hashed_password = self
                    .argon_util
                    .hash_password(password.as_str(), &salt)?;
            }
        }
